        ...
        xmlUtils.getOptionalValueByXPath(organisation, ORG_CODE_XPATH)
            .ifPresent(it -> org.addType(
                new CodeableConcept(new Coding()
                    .setCode(it))));
        xmlUtils.detachOptionalNodeByXPath(organisation, ORG_NAME_XPATH)
            .ifPresent(name -> org.setName(name.getTextContent()));

        xmlUtils.detachOptionalNodeByXPath(organisation, ADDRESS_XPATH)
            .ifPresent(node -> org.addAddress(new Address().setText(node.getTextContent())));

        xmlUtils.detachOptionalNodeByXPath(organisation, TELECOM_XPATH)
