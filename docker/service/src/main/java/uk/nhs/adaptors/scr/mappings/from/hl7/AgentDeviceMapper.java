...
public class AgentDeviceMapper implements XmlToFhirMapper {
    ...
    private Organization mapOrganization(Node agentDevice) {
        Organization org;
        Optional<Node> orgNode = xmlUtils.getOptionalNodeByXpathAndDetach(agentDevice, ORG_XPATH);
        Optional<Node> orgSdsNode = xmlUtils.getOptionalNodeByXpathAndDetach(agentDevice, ORG_SDS_XPATH);
    ...
    private Device mapDevice(Node agentDevice) {
        ...
        Optional<Node> deviceSds = xmlUtils.getOptionalNodeByXpathAndDetach(agentDevice, AGENT_DEVICE_SDS_XPATH);
        Optional<Node> deviceHl7 = xmlUtils.getOptionalNodeByXpathAndDetach(agentDevice, AGENT_DEVICE_XPATH);
