...
public class GpSummaryMapper implements XmlToFhirMapper {
    ...
    public List<Resource> map(Node document) {
        ...
        var presentationTextValue =
                xmlUtils.getOptionalNodeByXpathAndDetach(document, PRESENTATION_TEXT_VALUE);
    ...
    private void addAuthor(Node document, List<Resource> resources, Composition composition) {
        xmlUtils.getOptionalNodeByXpathAndDetach(document, GP_SUMMARY_AUTHOR_AGENT_PERSON_SDS_XPATH)
        ...
        xmlUtils.getOptionalNodeByXpathAndDetach(document, GP_SUMMARY_AUTHOR_AGENT_PERSON_XPATH)
                .ifPresent(agentPersonSds -> {
